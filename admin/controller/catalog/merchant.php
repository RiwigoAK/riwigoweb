...
class ControllerCatalogMerchant extends Controller {
	...
	protected function getForm() {  
		...
		// Mrerchant Discount  
		if (isset($this->request->post['merchant_discount'])) {
			$merchant_discounts = $this->request->post['merchant_discount'];
		} elseif (isset($this->request->get['merchant_id'])) {
			$merchant_discounts = $this->model_catalog_merchant->getMerchantDiscounts($this->request->get['merchant_id']);
		} else {
			$merchant_discounts = array();
		}

		$data['merchant_discounts'] = array();
		
		//print '<pre>'; print_r($merchant_discounts);exit;

		
                
                if(count($merchant_discounts)==0){
                    $selectedTime = "8:00"; $i=1;
                    for($x = 0; $x <= 31; $x++) {
                      ...
                      //  echo date('h:i', $endtime)."</br>";
                       ...
                       /* $data['merchant_discounts'][] = array(        
						...
						'merchant_time'      => date('h:i A', $endtime), 
						'sort_order'      	=> $i,
						'mon_percentage'   	=> '',
						'mon_packs'   		=> '', 
						'tue_percentage'   	=> '',
                   ...
                   // print '<pre>'; print_r($data['merchant_discounts']);exit;
                }else{
					
					foreach ($merchant_discounts as $merchant_discounts) {
			 			
						if(!isset($merchant_discounts['merchant_id'])){$merchant_discounts['merchant_id']=0;}
						
						$data['merchant_discounts'][] = array(
							'merchant_id'      	=> $merchant_discounts['merchant_id'],
							'sort_order'      	=> $merchant_discounts['sort_order'],
							'merchant_time'     => $merchant_discounts['merchant_time'], 
							'mon_percentage'   	=> $merchant_discounts['mon_percentage'],
							'mon_packs'   		=> $merchant_discounts['mon_packs'], 
							'tue_percentage'   	=> $merchant_discounts['tue_percentage'],
							'tue_packs'   		=> $merchant_discounts['tue_packs'], 
							'wed_percentage'   	=> $merchant_discounts['wed_percentage'],
							'wed_packs'   		=> $merchant_discounts['wed_packs'], 
							'thu_percentage'       => $merchant_discounts['thu_percentage'],
							'thu_packs'   		=> $merchant_discounts['thu_packs'], 
							'fri_percentage'   	=> $merchant_discounts['fri_percentage'],
							'fri_packs'   		=> $merchant_discounts['fri_packs'], 
							'sat_percentage'   	=> $merchant_discounts['sat_percentage'],
							'sat_packs'   		=> $merchant_discounts['sat_packs'], 
							'sun_percentage'   	=> $merchant_discounts['sun_percentage'],
							'sun_packs'   		=> $merchant_discounts['sun_packs']
						);
						
						//print $merchant_discounts['thu_packs'];
					}
				}
	...
	public function autocompleteServices() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_minute'])) {
			...
			$results = $this->model_catalog_merchant->getProducts($filter_data);
