...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		$sql = "SELECT DISTINCT *, pd.name AS name, p.image,  
				...
				LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.product_id = '" . (int)$product_id . "' 
	...
	public function getProfiles($product_id) {
        ...
        public function getMerchantsByProductsId($product_id,$city_id=10) {
		return $this->db->query("SELECT count(mp.merchant_id) as total FROM `" . DB_PREFIX . "merchant_products` mp LEFT JOIN `" . DB_PREFIX . "merchant_description` md on (mp.merchant_id = md.merchant_id) WHERE mp.product_id = " . (int)$product_id . " AND md.city_id = ".(int)$city_id." ")->row['total'];
	}
