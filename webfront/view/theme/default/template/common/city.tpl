<ul class="drop drop1">
  <li> <a href="#"><span id="selectCity"><?=$city_name?></span><i class="fa fa-caret-down" aria-hidden="true"></i></a>
    ...
    <ul class="level_before">
      ...
      <li onclick="changeCity('<?php echo $mycity['name'];?>','<?php echo $mycity['city_id'];?>')"><a href="#" title="<?php echo $mycity['name'];?>"><img src="<?php echo $mycity['image'];?>"><?php echo $mycity['name'];?></a></li>
      <?php }?>
    </ul>
    <?php } ?>
  </li>
...
<script>
 ...
 function changeCity(name, city_id){
	$('#selectCity').html(name);
	$('input[name=\'city_id\']').val(city_id);
	//alert(city_id);
	 $.ajax({
            url: 'index.php?route=common/city/city&city_id=' + city_id +'&redirect=',
            dataType: 'json',
            beforeSend: function() {
