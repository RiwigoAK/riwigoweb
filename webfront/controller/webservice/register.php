		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') ) {  //$json['data'] = ($_GET);
			...
			if($api_info){
                            ...
                            if($rest_var['type']=='social'){ //print 'santanu';
                                ...
                                if ( !isset($rest_var['email']) ||(strlen(utf8_decode($rest_var['email'])) > 96) || !filter_var($rest_var['email'], FILTER_VALIDATE_EMAIL)) {
                                ...
                                if ( !isset($rest_var['mobile']) ||(strlen(utf8_decode($rest_var['mobile'])) < 10) || (strlen(utf8_decode($rest_var['mobile'])) > 20)) {
                                        $json['mobile'] = $this->language->get('error_mobile');
                                }else if ($this->model_account_customer->getTotalCustomersByMobile($rest_var['mobile'])) {
                                ...
                                if ( !isset($rest_var['password']) ||(strlen(utf8_decode($rest_var['password'])) < 4) || (strlen(utf8_decode($rest_var['password'])) > 20)) {
