...
class ControllerAccountForgotten extends Controller {
	...
	public function validate() {
		...
		if (!isset($this->request->post['frmUserEmail'])) {
                    $json['warning'] = $this->language->get('error_email');
		} elseif (!$this->model_account_customer->getTotalCustomersByEmail($this->request->post['frmUserEmail'])) {
                    $json['warning'] = $this->language->get('error_email');
		}
		...
		if ($customer_info && !$customer_info['approved']) {
			...
			$this->model_account_customer->editCode($this->request->post['frmUserEmail'], $code);
			...
			$mail->setTo($this->request->post['frmUserEmail']);
			$mail->setFrom($this->config->get('config_frmUserEmail'));
			$mail->setSender(html_entity_decode($this->config->get('config_name'), ENT_QUOTES, 'UTF-8'));
			$mail->setSubject(html_entity_decode($subject, ENT_QUOTES, 'UTF-8'));
			$mail->setText(html_entity_decode($message, ENT_QUOTES, 'UTF-8'));
			//$mail->send();
